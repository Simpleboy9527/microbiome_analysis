...
for i in `less /beegfs/xuhuan/Strain/SRR_Acc_List.txt`;do
  fastq-dump --split-3 ${i}/${i}.sra --outdir raw_data
done
...
echo -e "sample-id\tforward-absolute-filepath\treverse-absolute-filepath" > manifest && for f in *_1.fastq; do echo -e "$(basename $f _1.fastq)\t$(realpath $f)\t$(realpath ${f/_1/_2})"; done >> manifest
